...
;(function(window, wp, shortcode, $) {
      ...
      _.each(views, function(view, type) {
        ...
        _.each(current, function(piece) {
          ...
          while (remaining && (result = view.prototype.match(remaining))) {
            ...
            pieces.push({
              content: instance.ignore
                ? text
                : '<p data-wpview-marker="' + instance.encodedText + '">' + text + '</p>',
              processed: true
            })
  ...
  _.extend(
    wp.mce.View.prototype,
    /** @lends wp.mce.View.prototype */ {
      /**
       * The content.
       *
       * @type {*}
      ...
      setContent: function(content, callback, rendered) {
        if (
          _.isObject(content) &&
          (content.sandbox || content.head || content.body.indexOf('<script') !== -1)
        ) {
      ...
      setIframes: function(head, body, callback, rendered) {
        ...
        if (body.indexOf('[') !== -1 && body.indexOf(']') !== -1) {
          var shortcodesRegExp = new RegExp(
            '\\[\\/?(?:' + window.mceViewL10n.shortcodes.join('|') + ')[^\\]]*?\\]',
            'g'
          )
        ...
        this.getNodes(function(editor, node) {
          ...
          MutationObserver =
            ...
            iframeWin.WebKitMutationObserver ||
            iframeWin.MozMutationObserver
...
;(function(window, views, media, $) {
  var base, gallery, av, embed, schema, parser, serializer
