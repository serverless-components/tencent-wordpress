  ...
  wpList = {
    ...
    nonce: function(element, settings) {
      ...
      return (
        settings.nonce ||
        url._ajax_nonce ||
        $element.find('input[name="_ajax_nonce"]').val() ||
        url._wpnonce ||
        $element.find('input[name="_wpnonce"]').val() ||
        0
      )
    },
    ...
    ajaxAdd: function(element, settings) {
      ...
      formValues = $('#' + settings.element + ' :input').not(
        '[name="_ajax_nonce"], [name="_wpnonce"], [name="action"]'
      )
      formData = $.isFunction(formValues.fieldSerialize)
      ...
      settings.success = function(response) {
        ...
        if (!parsedResponse || parsedResponse.errors) {
          $eventTarget
            .stop()
            .stop()
            .css('backgroundColor', '#ff3333')
            [isClass ? 'removeClass' : 'addClass'](settings.dimClass)
            .show()
            .queue(function() {
              list.wpList.recolor()
              $(this).dequeue()
        ...
        if ('undefined' !== typeof parsedResponse.responses[0].supplemental.comment_link) {
          ...
          if ('' !== parsedResponse.responses[0].supplemental.comment_link) {
            $submittedOn.html(
              $('<a></a>')
                .text($submittedOn.text())
                .prop('href', parsedResponse.responses[0].supplemental.comment_link)
            )
    ...
    add: function(element, settings) {
      ...
      if (settings.alt) {
        $element.toggleClass(
          settings.alt,
          ($list.children(':visible').index($element[0]) + settings.altOffset) % 2
        )
      }
    ...
    process: function(elementId) {
      ...
      $element.on(
        'click',
        'a[data-wp-lists^="add:' + list.id + ':"], input[data-wp-lists^="add:' + list.id + ':"]',
        function() {
          return list.wpList.add(this)
        }
      )
    ...
    recolor: function() {
      ...
      items
        .filter(evenOdd[0])
        .addClass(list.wpList.settings.alt)
        .end()
    ...
    this.each(function(index, list) {
      list.wpList = {
        settings: $.extend(
          {},
          wpList.settings,
          { what: wpList.parseData(list, 'list')[1] || '' },
          settings
        )
      }
