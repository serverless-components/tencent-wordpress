;(function() {
  var inlite = (function(domGlobals) {
    ...
    global$4.each('fire on off show hide append prepend before after reflow'.split(' '), function(
      name
    ) {
      proto[name] = function() {
    ...
    function toggleBodyFullScreenClasses(classPrefix, state) {
      if (checkFullscreenWindows() && state === false) {
        global$7([domGlobals.document.documentElement, domGlobals.document.body]).removeClass(
      ...
      fullscreen: function(state) {
        ...
        if (state !== self._fullscreen) {
          ...
          if (!state) {
            ...
            global$7([documentElement, domGlobals.document.body]).removeClass(prefix + 'fullscreen')
            ...
            global$7([documentElement, domGlobals.document.body]).addClass(prefix + 'fullscreen')
      ...
      var showForm = function(editor, id) {
        if (panel) {
          ...
          if (result) {
            ...
            movePanelTo(
              panel,
              Layout.userConstrain(userConstainHandler, currentRect, contentAreaRect, panelRect)
            )
            togglePositionClass(panel, result.position)
          }
        }
