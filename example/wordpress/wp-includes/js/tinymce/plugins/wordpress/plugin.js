  ...
  tinymce.PluginManager.add('wordpress', function(editor) {
    ...
    function toggleToolbars(state) {
      ...
      if (state === 'hide') {
        ...
        classicBlockToolbar.length &&
        !classicBlockToolbar.hasClass('has-advanced-toolbar')
      ) {
    ...
    editor.on('BeforeSetContent', function(event) {
      ...
      if (event.content) {
        if (event.content.indexOf('<!--more') !== -1) {
          ...
          event.content = event.content.replace(/<!--more(.*?)-->/g, function(match, moretext) {
            return (
              ...
              moretext +
              '" ' +
              'class="wp-more-tag mce-wp-more" alt="" title="' +
              title +
        ...
        if (event.content.indexOf('<script') !== -1 || event.content.indexOf('<style') !== -1) {
          event.content = event.content.replace(/<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function(
            match,
            tag
          ) {
            return (
    ...
    editor.addCommand('WP_Help', function() {
      ...
      dialog = editor.windowManager.open({
        title: editor.settings.classic_block_editor
    ...
    editor.on('preInit', function() {
      var validElementsSetting =
        '@[id|accesskey|class|dir|lang|style|tabindex|' +
      ...
      if (tinymce.Env.mac) {
        tinymce
          ...
          .attr('title', __('Rich Text Area. Press Control-Option-H for help.'))
      }
    })
