...
wp.customize.navMenusPreview = wp.customize.MenusCustomizerPreview = (function($, _, wp, api) {
  ...
  if (api.selectiveRefresh) {
    ...
    self.NavMenuInstancePartial = api.selectiveRefresh.Partial.extend(
      /** @lends wp.customize.navMenusPreview.NavMenuInstancePartial.prototype */ {
        /**
         * Constructor.
         *
         * @since 4.5.0
        ...
        initialize: function(id, options) {
          ...
          if (!matches) {
            throw new Error(
              'Illegal id for nav_menu_instance partial. The key corresponds with the args HMAC.'
            )
          }
          argsHmac = matches[1]
        ...
        isRelatedSetting: function(setting, newValue, oldValue) {
          ...
          if (partial.params.navMenuArgs.theme_location) {
            if (
              'nav_menu_locations[' + partial.params.navMenuArgs.theme_location + ']' ===
              setting.id
            ) {
              return true
            }
        ...
        refresh: function() {
          ...
          if (_.isNumber(partial.params.navMenuArgs.menu)) {
            ...
            partial.params.navMenuArgs.theme_location &&
            api.has('nav_menu_locations[' + partial.params.navMenuArgs.theme_location + ']')
          ) {
            menuId = api(
              'nav_menu_locations[' + partial.params.navMenuArgs.theme_location + ']'
            ).get()
        ...
        renderContent: function(placement) {
          var partial = this,
            previousContainer = placement.container
  ...
  self.highlightControls = function() {
    ...
    $(document).on('click', selector, function(e) {
      ...
      navMenuItemParts = $(this)
        .attr('class')
        .match(/(?:^|\s)menu-item-(-?\d+)(?:\s|$)/)
      if (navMenuItemParts) {
        e.preventDefault()
