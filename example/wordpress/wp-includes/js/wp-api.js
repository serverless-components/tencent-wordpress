      ...
      if (_.includes(routeEndpoint.methods, 'POST') || _.includes(routeEndpoint.methods, 'PUT')) {
        ...
        if (!_.isEmpty(routeEndpoint.args)) {
          ...
          if (_.isEmpty(modelInstance.prototype.args)) {
            ...
            modelInstance.prototype.args = _.extend(
              modelInstance.prototype.args,
              routeEndpoint.args
            )
          }
        }
        ...
        if (_.includes(routeEndpoint.methods, 'GET')) {
          ...
          if (!_.isEmpty(routeEndpoint.args)) {
            ...
            if (_.isEmpty(modelInstance.prototype.options)) {
              ...
              modelInstance.prototype.options = _.extend(
...
;(function() {
    ...
    /** @lends Endpoint.prototype */ {
      ...
      initialize: function() {
        ...
        if (model.get('schema')) {
          ...
          sessionStorage.getItem(
            'wp-api-schema-model' + model.get('apiRoot') + model.get('versionString')
          )
        ) {
          // Used a cached copy of the schema model if available.
          model.schemaModel.set(
            model.schemaModel.parse(
              JSON.parse(
                sessionStorage.getItem(
                  'wp-api-schema-model' + model.get('apiRoot') + model.get('versionString')
                )
      ...
      constructFromSchema: function() {
        ...
        _.each(routeModel.schemaModel.get('routes'), function(route, index) {
          ...
          if (
            index !== routeModel.get(' versionString') &&
            index !== schemaRoot &&
        ...
        _.each(modelRoutes, function(modelRoute) {
          ...
          var modelClassName,
            routeName = wp.api.utils.extractRoutePart(
              modelRoute.index,
              2,
              routeModel.get('versionString'),
              true
            ),
            parentName = wp.api.utils.extractRoutePart(
              modelRoute.index,
              1,
              routeModel.get('versionString'),
              false
            ),
            routeEnd = wp.api.utils.extractRoutePart(
              modelRoute.index,
              1,
              routeModel.get('versionString'),
              true
          ...
          if ('' !== parentName && parentName !== routeName) {
            modelClassName =
              wp.api.utils.capitalizeAndCamelCaseDashes(parentName) +
              wp.api.utils.capitalizeAndCamelCaseDashes(routeName)
        ...
        _.each(collectionRoutes, function(collectionRoute) {
          ...
          if (
            ...
            routeModel.get('versionString') !== parentName
          ) {
            collectionClassName =
              wp.api.utils.capitalizeAndCamelCaseDashes(parentName) +
              wp.api.utils.capitalizeAndCamelCaseDashes(routeName)
          ...
          wp.api.utils.decorateFromRoute(
            ...
            loadingObjects.collections[collectionClassName]
          )
        })

        // Add mixins and helpers for each of the models.
        _.each(loadingObjects.models, function(model, index) {
          loadingObjects.models[index] = wp.api.utils.addMixinsAndHelpers(
            model,
            index,
            loadingObjects
          )
  ...
  wp.api.init = function(args) {
    ...
    if (
      !attributes.schema &&
      attributes.apiRoot === wpApiSettings.root &&
      attributes.versionString === wpApiSettings.versionString
    ) {
