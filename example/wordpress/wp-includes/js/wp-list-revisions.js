  ...
  var init = function() {
    ...
    pr.onclick = function() {
      ...
      for (i = 0; i < inputs.length; i++) {
        ...
        if (
          !inputs[i].checked &&
          (('left' == side && 1 > checkCount) ||
            ('right' == side && 1 < checkCount && (!inputs[i - 1] || !inputs[i - 1].checked))) &&
          !(inputs[i + 1] && inputs[i + 1].checked && 'right' == inputs[i + 1].getAttribute('name'))
        )
          inputs[i].style.visibility = 'hidden'
        else if ('left' == side || 'right' == side) inputs[i].style.visibility = 'visible'
      }
    }
    pr.onclick()
  }
  if (w && w.addEventListener) w.addEventListener('load', init, false)
  else if (w && w.attachEvent) w.attachEvent('onload', init)
})(window)
