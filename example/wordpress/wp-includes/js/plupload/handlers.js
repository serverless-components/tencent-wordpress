...
function updateMediaForm() {
  ...
  if (items.not('.media-blank').length > 0) jQuery('.savebutton').show()
  else jQuery('.savebutton').hide()
}
...
function prepareMediaItem(fileObj, serverData) {
  ...
  if (f == 2 && shortform > 2) f = shortform

  try {
    if (typeof topWin.tb_remove != 'undefined') topWin.jQuery('#TB_overlay').click(topWin.tb_remove)
  } catch (e) {}
...
function deleteSuccess(data) {
  ...
  if (
    jQuery('form.type-form #media-items').children().length == 1 &&
    jQuery('.hidden', '#media-items').length > 0
  ) {
  ...
  jQuery('.slidetoggle', item)
    .slideUp(200)
    .siblings()
    .removeClass('hidden')
  item
    .css({ backgroundColor: '#faa' })
    .animate({ backgroundColor: '#f4f4f4' }, { queue: false, duration: 500 })
    .addClass('undo')

  jQuery('.filename:empty', item).remove()
  jQuery('.filename .title', item).css('font-weight', 'bold')
  jQuery('.filename', item)
    .append('<span class="trashnotice"> ' + pluploadL10n.deleted + ' </span>')
    .siblings('a.toggle')
    .hide()
...
function switchUploader(s) {
  if (s) {
    ...
    if (typeof uploader == 'object') uploader.refresh()
  } else {
...
function wpFileExtensionError(up, file, message) {
  jQuery('#media-items').append(
    '<div id="media-item-' + file.id + '" class="media-item error"><p>' + message + '</p></div>'
  )
  up.removeFile(file)
}
...
jQuery(document).ready(function($) {
  ...
  $('.media-upload-form').bind('click.uploader', function(e) {
    ...
    if (target.is('input[type="radio"]')) {
      // Remember the last used image size and alignment.
      tr = target.closest('tr')

      if (tr.hasClass('align')) setUserSetting('align', target.val())
      else if (tr.hasClass('image-size')) setUserSetting('imgsize', target.val())
    } else if (target.is('button.button')) {
      // Remember the last used image link url.
    ...
    } else if (target.is('.upload-flash-bypass a') || target.is('a.uploader-html')) {
      // Switch uploader to html4.
      $('#media-items, p.submit, span.big-file-warning').css('display', 'none')
      switchUploader(0)
      e.preventDefault()
    } else if (target.is('.upload-html-bypass a')) {
      // Switch uploader to multi-file.
      $('#media-items, p.submit, span.big-file-warning').css('display', '')
      switchUploader(1)
      e.preventDefault()
    } else if (target.is('a.describe-toggle-on')) {
      // Show.
      ...
      target.siblings('.slidetoggle').fadeIn(250, function() {
        ...
        if (H && top && h) {
          ...
          if (b > B) {
            if (b - B < top - S) window.scrollBy(0, b - B + 10)
            else window.scrollBy(0, top - S - 40)
          }
        }
      })

      e.preventDefault()
    } else if (target.is('a.describe-toggle-off')) {
      // Hide.
