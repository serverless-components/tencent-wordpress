!(function(r) {
  ...
  var p = /\r?\n/g,
    w = {}
  ;(w.fileapi = void 0 !== q('<input type="file">').get(0).files),
    (w.formdata = void 0 !== window.FormData)
  var _ = !!q.fn.prop
  function i(e) {
    var t = e.data
  ...
  ;(q.fn.attr2 = function() {
    ...
    (q.fn.ajaxSubmit = function(M, e, t, r) {
      ...
      if (
        ((M.success = function(e, t, r) {
          for (var a = M.context || this, n = 0, i = f.length; n < i; n++)
            f[n].apply(a, [e, t, r || X, X])
        }),
        M.error)
      ) {
        var p = M.error
      ...
      !1 !== M.iframe && (M.iframe || b)
        ...
        : (y =
            (h || g) && x
              ...
              : q.ajax(M)),
        X.removeData('jqxhr').data('jqxhr', y)
      ...
      function j(e) {
        ...
        if (
          (l.iframeTarget
            ? (n = (d = q(l.iframeTarget, s)).attr2('name'))
              ? (i = n)
              : d.attr2('name', i)
            : (d = q('<iframe name="' + i + '" src="' + l.iframeSrc + '" />', s)).css({
                position: 'absolute',
                top: '-1000px',
                left: '-1000px'
              }),
          ...
          (m = {
            ...
            abort: function(e) {
              ...
              d.attr('src', l.iframeSrc),
                (m.error = t),
                l.error && l.error.call(l.context, m, t, e),
                f && q.event.trigger('ajaxError', [m, l, t]),
                l.complete && l.complete.call(l.context, m, t)
            }
          }),
          (f = l.global) && 0 == q.active++ && q.event.trigger('ajaxStart'),
          f && q.event.trigger('ajaxSend', [m, l]),
        ...
        ;(a = o.clk) &&
          ...
          'image' === a.type &&
            ((l.extraData[n + '.x'] = o.clk_x), (l.extraData[n + '.y'] = o.clk_y)))
        var x = 1,
          y = 2
        function b(t) {
          var r = null
        ...
        function j() {
          ...
          o.setAttribute('target', i),
            (O && !/post/i.test(O)) || o.setAttribute('method', 'POST'),
            t !== l.url && o.setAttribute('action', l.url),
          ...
          try {
            ...
            l.iframeTarget || d.appendTo(u),
              p.attachEvent ? p.attachEvent('onload', A) : p.addEventListener('load', A, !1),
            ...
            o.setAttribute('action', t),
              ...
              e ? o.setAttribute('target', e) : X.removeAttr('target'),
              q(a).remove()
          }
        }
        ...
        function A(e) {
          if (!m.aborted && !k) {
            if (((S = b(p)) || (N('cannot access response document'), (e = y)), e === x && m))
              return m.abort('timeout'), void g.reject(m, 'timeout')
            if (e === y && m)
              return m.abort('server abort'), void g.reject(m, 'error', 'server abort')
            if ((S && S.location.href !== l.iframeSrc) || h) {
              ...
              try {
                ...
                if (o || l.textarea) {
                  ...
                  if (s)
                    ...
                    u
                      ? (m.responseText = u.textContent ? u.textContent : u.innerText)
                      : c && (m.responseText = c.textContent ? c.textContent : c.innerText)
                  }
                } else
                  'xml' === i &&
                    !m.responseXML &&
                    m.responseText &&
                    (m.responseXML = L(m.responseText))
                try {
                  w = E(m, i, l)
                } catch (e) {
              ...
              m.aborted && (N('upload aborted'), (r = null)),
                m.status &&
                  (r =
                    (200 <= m.status && m.status < 300) || 304 === m.status ? 'success' : 'error'),
                'success' === r
                  ? (l.success && l.success.call(l.context, w, 'success', m),
                    g.resolve(m.responseText, 'success', m),
                    f && q.event.trigger('ajaxSuccess', [m, l]))
                  : r &&
                    (void 0 === t && (t = m.statusText),
                    l.error && l.error.call(l.context, m, r, t),
                    g.reject(m, 'error', t),
                    f && q.event.trigger('ajaxError', [m, l, t])),
                f && q.event.trigger('ajaxComplete', [m, l]),
                f && !--q.active && q.event.trigger('ajaxStop'),
                l.complete && l.complete.call(l.context, m, r),
                (k = !0),
        ...
        var L =
          ...
          E = function(e, t, r) {
            ...
            return (
              n &&
                'parsererror' === i.documentElement.nodeName &&
                q.error &&
                q.error('parsererror'),
              ...
              'string' == typeof i &&
                (('json' === t || !t) && 0 <= a.indexOf('json')
                  ? (i = F(i))
                  : ('script' === t || !t) && 0 <= a.indexOf('javascript') && q.globalEval(i)),
              i
            )
          }
        return g
    ...
    (q.fn.ajaxForm = function(e, t, r, a) {
      if (
        ...
        return e.delegation
          ? (q(document)
              ...
              .on('submit.form-plugin', this.selector, e, i)
              .on('click.form-plugin', this.selector, e, o),
            this)
          : this.ajaxFormUnbind()
              .on('submit.form-plugin', e, i)
              .on('click.form-plugin', e, o)
      ...
      return (
        !q.isReady && n.s
          ...
          : N(
              'terminating; zero elements found by selector' + (q.isReady ? '' : ' (DOM not ready)')
            ),
        this
      )
    }),
    ...
    (q.fn.formToArray = function(e, t, r) {
      ...
      for (q.isFunction(r) && (m = q.map(m, r)), i = 0, l = m.length; i < l; i++)
        if ((s = (c = m[i]).name) && !c.disabled)
          if (e && d.clk && 'image' === c.type)
            d.clk === c &&
              (a.push({ name: s, value: q(c).val(), type: c.type }),
              a.push({ name: s + '.x', value: d.clk_x }, { name: s + '.y', value: d.clk_y }))
          else if ((u = q.fieldValue(c, !0)) && u.constructor === Array)
            for (t && t.push(c), o = 0, f = u.length; o < f; o++) a.push({ name: s, value: u[o] })
          else if (w.fileapi && 'file' === c.type) {
            t && t.push(c)
            var h = c.files
            if (h.length)
              for (o = 0; o < h.length; o++) a.push({ name: s, value: h[o], type: c.type })
            else a.push({ name: s, value: '', type: c.type })
          } else
            null != u &&
              (t && t.push(c), a.push({ name: s, value: u, type: c.type, required: c.required }))
      if (!e && d.clk) {
        var v = q(d.clk),
          g = v[0]
        ;(s = g.name) &&
        ...
        this.each(function() {
          ...
          if (e) {
            ...
            if (t && t.constructor === Array)
              for (var r = 0, a = t.length; r < a; r++) i.push({ name: e, value: t[r] })
    ...
    (q.fn.fieldValue = function(e) {
      for (var t = [], r = 0, a = this.length; r < a; r++) {
        ...
        null == i ||
          ...
          (i.constructor === Array ? q.merge(t, i) : t.push(i))
      }
      return t
    }),
    (q.fieldValue = function(e, t) {
      ...
      if (i < 0) return null
      for (
        var o = [], s = e.options, u = 'select-one' === a, c = u ? i + 1 : s.length, l = u ? i : 0;
        l < c;
        l++
      ) {
        var f = s[l]
      ...
      return this.each(function() {
        ...
        a.test(e) || 'textarea' === t
          ...
          ? /MSIE/.test(navigator.userAgent)
            ...
            : q(this).val('')
          : r &&
            ((!0 === r && /hidden/.test(e)) || ('string' == typeof r && q(this).is(r))) &&
            (this.value = '')
      })
    }),
          ...
          case 'select':
            return (
              t.find('option').each(function(e) {
                if (
                  ((this.selected = this.defaultSelected), this.defaultSelected && !t[0].multiple)
                )
      ...
      return (
        ...
        this.each(function() {
          ...
          if ('checkbox' === e || 'radio' === e) this.checked = r
            ...
            r && t[0] && 'select-one' === t[0].type && t.find('option').selected(!1),
              (this.selected = r)
          }
        })
      )
