...
function getAnchorWindowPosition(anchorname) {
  ...
  if (document.getElementById) {
    ...
    x =
      coordinates.x + window.screenX + (window.outerWidth - window.innerWidth) - window.pageXOffset
    y =
      coordinates.y +
      window.screenY +
      (window.outerHeight - 24 - window.innerHeight) -
...
function pickColor(color) {
  if (ColorPicker_targetInput == null) {
    alert(
      "Target Input is null, which means you either didn't use the 'select' function or you have no defined your own 'pickColor' function to handle the picked color!"
    )
    return
  }
...
function ColorPicker() {
  ...
  if (windowMode) {
    ...
    cp_contents +=
      "<body marginwidth=0 marginheight=0 leftmargin=0 topmargin=0><span style='text-align: center;'>"
  }
  ...
  for (var i = 0; i < total; i++) {
    ...
    cp_contents +=
      ...
      windowRef +
      "ColorPicker_pickColor('" +
      colors[i] +
      "'," +
      windowRef +
      'window.popupWindowObjects[' +
      cp.index +
      ']);return false;" ' +
      mo +
      '>&nbsp;</a></td>'
  ...
  if (document.getElementById) {
    ...
    cp_contents +=
      "<tr><td colspan='" +
      width1 +
      "' style='background-color: #FFF;' ID='colorPickerSelectedColor'>&nbsp;</td><td colspan='" +
      width2 +
      "' style='text-align: center;' id='colorPickerSelectedColorValue'>#FFFFFF</td></tr>"
  }
  cp_contents += '</table>'
  if (windowMode) {
