...
;(function($) {
  ...
  $.imgAreaSelect = function(img, options) {
    ...
    function update(resetKeyPress) {
       ...
       */
      $box
        .css({ left: viewX(selection.x1), top: viewY(selection.y1) })
        .add($area)
        .width((w = selection.width))
        .height((h = selection.height))
    ...
    function fixAspectRatio(xFirst) {
      if (aspectRatio)
        if (xFirst) {
          ...
          y2 = round(
            max(top, min(top + imgHeight, y1 + (abs(x2 - x1) / aspectRatio) * (y2 > y1 || -1)))
          )
          x2 = round(x2)
        } else {
          y2 = max(top, min(top + imgHeight, y1 + (abs(x2 - x1) / aspectRatio) * (y2 > y1 || -1)))
          x2 = round(
            max(left, min(left + imgWidth, x1 + abs(y2 - y1) * aspectRatio * (x2 > x1 || -1)))
          )
          y2 = round(y2)
        }
    }
    ...
    function cancelSelection() {
      $(document)
        .off('mousemove touchmove', startSelection)
        .off('mouseup touchend', cancelSelection)
      hide($box.add($outer))
    ...
    function imgMouseDown(event) {
      ...
      $(document).on({ 'mousemove touchmove': startSelection, 'mouseup touchend': cancelSelection })
    ...
    var docKeyPress = function(event) {
      ...
      d =
        !isNaN(k.alt) && (event.altKey || event.originalEvent.altKey)
          ? k.alt
    ...
    function setOptions(newOptions) {
      ...
      if (options.disable || options.enable === false) {
        /* Disable the plugin */
        $box.off({ 'mousemove touchmove': areaMouseMove, 'mousedown touchstart': areaMouseDown })
        $(window).off('resize', windowResize)
      } else {
        if (options.enable || options.disable === false) {
          /* Enable the plugin */
          if (options.resizable || options.movable)
            $box.on({ 'mousemove touchmove': areaMouseMove, 'mousedown touchstart': areaMouseDown })
    ...
    $box
      .add($outer)
      .css({ visibility: 'hidden', position: position, overflow: 'hidden', zIndex: zIndex || '0' })
