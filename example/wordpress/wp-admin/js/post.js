...
jQuery(document).ready(function($) {
   ...
   */
  $(window)
    .on('beforeunload.edit-post', function() {
      var editor = typeof tinymce !== 'undefined' && tinymce.get('content')

      if (
        (editor && !editor.isHidden() && editor.isDirty()) ||
  ...
  if ($('#submitdiv').length) {
    ...
    $timestampdiv.find('.save-timestamp').click(function(event) {
      // Crazyhorse - multiple OK cancels.
      if (updateText()) {
  ...
  function editPermalink() {
    ...
    $el
      .html(
        '<input type="text" id="new-post-slug" value="' + slug_value + '" autocomplete="off" />'
      )
      .children('input')
      .keydown(function(e) {
        var key = e.which
