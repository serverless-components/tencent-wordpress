jQuery(function($) {
  ...
  $('.akismet-user-comment-count').each(function() {
    var thisId = $(this).attr('commentid')
    $(this)
      .insertAfter('#comment-' + thisId + ' .author strong:first')
      .show()
  })
  ...
  $('#the-comment-list')
    .on('click', '.akismet_remove_url', function() {
      var thisId = $(this).attr('commentid')
      var data = {
      ...
      $.ajax({
        ...
        beforeSend: function() {
          ...
          // Show temp status
          $('#author_comment_url_' + thisId).html(
            $('<span/>').text(WPAkismet.strings['Removing...'])
          )
        },
        success: function(response) {
          if (response) {
            ...
            $('#author_comment_url_' + thisId)
              ...
              .html($('<span/>').text(WPAkismet.strings['URL removed']))
              .append(' ')
              .append(
                $('<span/>')
    ...
    .on('click', '.akismet_undo_link_removal', function() {
      ...
      $.ajax({
        ...
        beforeSend: function() {
          // Show temp status
          $('#author_comment_url_' + thisId).html(
            $('<span/>').text(WPAkismet.strings['Re-adding...'])
          )
        },
        success: function(response) {
          if (response) {
            ...
            $('#author_comment_url_' + thisId)
              .removeClass('akismet_undo_link_removal')
              .text(thisUrl.replace(/^http:\/\/(www\.)?/gi, ''))
          }
        }
      })
  ...
  if ('enable_mshots' in WPAkismet && WPAkismet.enable_mshots) {
    $('#the-comment-list')
      .on('mouseover', mshotEnabledLinkSelector, function() {
        ...
        var mShot = $(
          '<div class="akismet-mshot mshot-container"><div class="mshot-arrow"></div><img src="' +
            akismet_mshot_url(thisHref) +
  ...
  $('.checkforspam.enable-on-load').click(function(e) {
    ...
    if ($(this).hasClass('button-disabled')) {
      window.location.href = $(this)
        .data('success-url')
        .replace('__recheck_count__', 0)
        .replace('__spam_count__', 0)
      return
    }
  ...
  function akismet_check_for_spam(offset, limit) {
    ...
    var percentage_complete =
      Math.round((recheck_count / check_for_spam_buttons.data('pending-comment-count')) * 1000) / 10

    // Update the progress counter on the "Check for Spam" button.
    $('.checkforspam').text(
      check_for_spam_buttons.data('progress-label').replace('%1$s', percentage_complete)
    )
    ...
    $.post(
      ...
      function(result) {
        ...
        if (result.counts.processed < limit) {
          window.location.href = check_for_spam_buttons
            .data('success-url')
            .replace('__recheck_count__', recheck_count)
            .replace('__spam_count__', spam_count)
        } else {
  ...
  if (typeof MutationObserver !== 'undefined') {
    ...
    if (comment_list_container) {
      ...
      observer.observe(comment_list_container, {
        attributes: true,
        childList: true,
        characterData: true
      })
    ...
    $('#the-comment-list')
      ...
      .each(function() {
        if (
          $(this)
            .parent()
            .find('.akismet_remove_url').length > 0
        ) {
          return
        }
